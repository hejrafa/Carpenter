...
-- Resolve spell ID to canonical entry (follow parent chain); returns type or nil
function addon.GetSpellInfo(spellId)
    if not spellId or not addon.Spells then return nil end
    local entry = addon.Spells[spellId]
    while entry and entry.parent do
        spellId = entry.parent
        entry = addon.Spells[spellId]
    end
    return entry and entry.type or nil
end

-- Follow parent chain and return the root spell ID (for lookups)
function addon.GetRootSpellId(spellId)
    if not spellId or not addon.Spells then return spellId end
    local entry = addon.Spells[spellId]
    while entry and entry.parent do
        spellId = entry.parent
        entry = addon.Spells[spellId]
...
-- True if this spell ID should be shown as an important debuff (CC, root)
